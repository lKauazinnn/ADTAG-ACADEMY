...
const musicColors: Record<number, { color: string; dark: string; abbr: string }> = {
  20: { color: '#FF9A00', dark: '#5c3600', abbr: 'TC' },
  21: { color: '#9333ea', dark: '#3b1060', abbr: 'PR' },
  22: { color: '#31C5F4', dark: '#0b4458', abbr: 'MX' },
  23: { color: '#E1306C', dark: '#5c0e27', abbr: 'VZ' },
  24: { color: '#1DB954', dark: '#0a3d1c', abbr: 'RD' },
...
const Musicos: React.FC = () => {
  ...
  const fs = featuredMod ? (musicColors[featuredMod.order] ?? fallback) : fallback;

  if (loading) return (
    <div className="min-h-screen" style={{ background: '#050509' }}>
      <Header />
      <div className="flex items-center justify-center h-96">
        <div className="w-7 h-7 border-[3px] border-t-transparent rounded-full animate-spin" style={{ borderColor: '#FF9A00', borderTopColor: 'transparent' }} />
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen" style={{ background: '#050509' }}>
      <div className="pointer-events-none fixed inset-0 z-0" style={{ overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 110% 70% at 85% -10%, rgba(255,154,0,0.2), transparent 55%), radial-gradient(ellipse 90% 80% at -10% 65%, rgba(147,51,234,0.2), transparent 55%)' }} />
      ...
      <div className="relative z-10 overflow-hidden" style={{ background: 'rgba(5,5,12,0.55)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute bottom-0 left-0 right-0" style={{ height: 1, background: 'linear-gradient(90deg, transparent, rgba(255,154,0,0.5), rgba(147,51,234,0.5), transparent)' }} />
        </div>
        <div className="max-w-5xl mx-auto px-6 py-14 relative">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-10">
            ...
            <div className="flex items-center gap-3">
              {[{ val: modules.length, label: 'módulos', color: '#fafafa' }, { val: totalAulas, label: 'aulas', color: '#FF9A00' }, { val: '23h+', label: 'conteúdo', color: '#9333ea' }].map(s => (
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-10">
        ...
        {featuredMod && (
          <div className="mb-10 animate-fade-in-up" style={{ animationDelay: '0.05s' }}>
            ...
            >
              <div style={{ height: 3, background: `linear-gradient(90deg, ${fs.color}, #9333ea, #31C5F4)` }} />
              ...
              <div className="p-7 relative z-10 flex flex-col md:flex-row gap-8">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: fs.color }}>Músicos</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {modules.map((mod, index) => {
            const s = musicColors[mod.order] ?? fallback;
            return (
